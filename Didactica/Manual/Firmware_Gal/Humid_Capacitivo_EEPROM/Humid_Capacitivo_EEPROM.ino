...
//****************************************************************
byte Memor;    // Si é 255 indicará que non se grabou na EEPROM
int EMemor = 0;  // Enderezo
...
void setup() {
  ...
  pinMode (Buz, OUTPUT);
//****************************************************************
  if (EEPROM.read(EMemor) != 255) { // Comproba si se tem valores Actualizados na EEPROM
    EEPROM.get(EVSeco, VSeco);     // Pasa os valores as variables
    EEPROM.get(EVHumi, VHumi);
    Serial.print("Valores almacenados"); // Envia os valores almacenados vía serie
    visu();
  }
//****************************************************************
}
...
//****************************************************************
  EEPROM.get(EMemor, Memor);        // Recuperamos o valor de255 - as veces que se grabaron os datos
  Memor--;                          // Decrementamos o valor
  ...
  EEPROM.put(EVHumi, VHumi);        // Actualizamos o valor Humído na memoria EEPROM
  // Pasamos os valores... via serie.
